...
/*Funkcija za naogjanje na evklidovo rastojanie*/
function euclideanDistance(x1, y1, x2, y2) {

    return Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
}
/*Funkcija za presmetuvanje pet najbliski hoteli*/
function getNearestFive() {
    ...
    products.forEach(product => {
        ...
        if (nearest.length < 5) {
            ...
            nearest.push(product);
        } else {

            let index = Number.MAX_VALUE;
            let value = Number.MIN_VALUE;
            for (var i = 0; i < nearest.length; i++) {
                let productCords = product.coordinates.split(" ");
                let nearProductCords = nearest[i].coordinates.split(" ");
                let distanceProduct = euclideanDistance(productCords[0], productCords[1], x, y);
                let distanceNearProduct = euclideanDistance(nearProductCords[0], nearProductCords[1], x, y);
                if (distanceProduct < distanceNearProduct && distanceNearProduct>value) {
                    index = i;
                    value=distanceNearProduct;
                }
            }
            if (index != Number.MAX_VALUE) {
                nearest[index] = product;
                console.log(product);
            }
        }
    });
